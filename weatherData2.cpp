...
void WeatherData::retrieveWeatherInfo(ptree &ptRoot){
    
    
    
        cityCond = ptRoot.get<string>("currently.summary");
        temperature = ptRoot.get<string>("currently.temperature");
        cloudcover = ptRoot.get<string>("currently.cloudCover");
        nearestStorm = ptRoot.get<string>("currently.nearestStormDistance");
        
    
    BOOST_FOREACH(ptree::value_type &featVal, ptRoot.get_child("alerts")) {
        // "features" maps to a JSON array, so each child should have no name
        if ( !featVal.first.empty() )
            throw "\"alerts\" child node has a name";

        alertDescrip = featVal.second.get<string>("description");
        alertTit = featVal.second.get<string>("title");
        alertTim = featVal.second.get<string>("time");
        alertExp = featVal.second.get<string>("expires");
  ...
  //  cout<<"Temperature is currently: "<<temperature<<endl
  //  <<"The cloud cover percentage is: "<<cloudcover<<endl
  //  <<"The Closest Storm is "+nearestStorm+"Km"<<endl
  //  <<endl<<alertTit<<endl
  //  <<endl<<alertDescrip<<endl
  //  <<endl<<"Issuing time: "
  //  <<alertTim<<endl
 //   <<"Expires: "<<alertExp<<endl;
    return;
}
